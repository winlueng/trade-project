...
class LoginModel extends CommonModel
{
	...
	public function smsToVisitorVerify()
	{
	    ...
	    $exist = M('WechatVisitor')->where(['phone' => $phoneNumber, 'open_id' => ['neq', '']])->getField('id');
	    ...
	    $temlID = 60299;//腾讯云模版id

	    $params = [$verifyCode];//模版参数
	...
	public function getUserInfo($openid = '', $uid = '')
	{
		...
		$info['wallet']	= M('WasteBook')
							->where(['visitor_id' => $res['id'], 'relevance_id' => $this->pInfo['id'], 'pay_status' => '1'])
							->order('id desc')
							->getField('balance');
	...
	public function visitorAdd($data)
	{
		...
		try {
			if ($info = $this->getUserInfo($data->openid)) return $info;
