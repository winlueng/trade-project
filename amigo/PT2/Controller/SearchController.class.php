...
class SearchController extends CommonController
{
	...
	public function __construct()
	{
		...
		// dump($this->pInfo['sx_data_base']);exit;
		$this->XS  		= new \XS($this->pInfo['sx_data_base']);
	...
	public function goSearch()
	{
		...
		foreach ($res as $v) {
			...
			$info = M('Goods')->field('status, goods_logo, company_id')->where(['id' => $v->id])->find();
			if ($info['company_id'] != '0') {
				$company_info = M('Company')->field('web_postfix, template_id')->where(['id' => $info['company_id']])->find();
				$arr[$v->id]['web_postfix'] = $company_info['web_postfix'];
				$arr[$v->id]['template'] 	= M('TemplateChange')->where(['id' => $company_info['template_id']])->getField('template_path');
			}
			$arr[$v->id]['goods_logo'] = $info['goods_logo'];
			$arr[$v->id]['status'] = $info['status'];
		} 
		// dump($res);exit;
	...
	// 获取相关搜索词
	private function getCorrelation()
	{
