...
class CommonController extends Controller
{
	...
	public function __construct()
	{
		...
		$this->saveStatis('CompanyStatistics', $this->companyInfo['id']);
	...
	public function checkProjectExist()
	{
		...
		if (!$this->pInfo) {
			...
			$info = M('WriteInfo')
					->field('market_name, principal, phone, project_logo, email, address_id, address_info, abstruct, app_id, app_secret, mch_id, mch_sectet')
	...
	protected function selIsSave()
	{
		if ($this->userInfo['id']) 
		{
			...
			if (in_array($this->companyInfo['id'], $list)) {
				return 1;
			}else{
				return 0;
			}
