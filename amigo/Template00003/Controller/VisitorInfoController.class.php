...
class VisitorInfoController extends CommonController
{
    ...
    public function __construct()
    {
        ...
        $this->orderObj = new \Template00002\Model\OrderFormModel();
    ...
    public function myCenter()
    {
        ...
        $data['cartTotal'] = M('ShoppingCart')
					        ->join('__GOODS__ ON __GOODS__.status not in(-1,3) AND __SHOPPING_CART__.visitor_id='.$this->userInfo['id'].' AND __GOODS__.id=__SHOPPING_CART__.goods_id')
					        ->count();
        /**
         * 订单状态提示
         */
        for ($i=1; $i < 7; $i++) { 
            $count[$i] = $this->orderObj->orderList($i);
        }
        // dump($data);exit;
