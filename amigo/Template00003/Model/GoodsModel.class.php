...
class GoodsModel extends CommonModel
{
	...
	public function showGoodsList()
	{
		...
		$result['classifyList'] = M('Goods_classify')->field('id, classify_name')->where($where)->select();
	...
	public function showGoodsDetail($id = '')
	{
		$goodsId = I('get.id');
		...
		$goods['Info'] = $this->field($field)->where(['status'=>['not in','-1,3'], 'company_id' => $this->companyInfo['id'],'id' => $goodsId])->find();
	...
	public function getAttr()
	{
		...
		$goods['Info'] = $this->field($field)
						->where(['status'=>['not in','-1,3'], 'company_id' => $this->companyInfo['id'],'id' => I('get.id')])
						->find();
	...
	public function collectGoods()
	{
		...
		if (!$info && $status == '1') 
		{
			$result = $this->collectObj->add(['visitor_id' => $this->userInfo['id'], 'goods_collect' => json_encode([$goodsId])]);
			return $result;
		}else if((!$info || !$info['goods_collect']) && $status == '2'){
			return '非法操作';
		}else if(!$info['goods_collect'] && $status == '1'){
			return $this->collectObj->where($where)->setField('goods_collect',json_encode([$goodsId]));
		}
		...
		return $this->collectObj->where($where)->setField('goods_collect',$data);
	}
}