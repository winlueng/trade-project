...
class NewsModel extends CommonModel
{
	...
	{
		$result = $this
		        ->field('id, news_logo, news_name, title, video, is_video, addtime, img_path, goods_id')
				->where([
					'id' => I('get.id'),
					'status' => ['neq', '-1'],
					'company_id' => $this->companyInfo['id'],
		...
		$result['visit_total']   = M('NewsStatistics')->where(['relevance_id' => $v['id']])->sum('visit_total');
		$result['goods_info']  = M('Goods')->field('id, price, goods_name, is_promotion, promotion_price, discount, goods_logo, status, sales_end_time, sales_start_time')
		                        ->where(['id' => $result['goods_id']])
		                        ->find();
		if ($result) {
			$this->saveStatis('NewsStatistics', $result['id']);
		}
		return $result;
	}
