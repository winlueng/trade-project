...
class ProjectBannerModel extends CommonModel
{
	...
	public function __construct()
	{
		...
		$this->_auto = [
			...
			array('project_id', $this->userInfo['relevance_id'], 1 ),
		];
	}
	...
	public function showList()
	{
		$where = [
			'status' => ['neq', '-1'],
			'project_id' => $this->userInfo['relevance_id']
		];
		...
		foreach ($list as $v) {
			$v['visit_total'] = M('ProjectBannerStatistics')->where(['relevance_id' => $v['id']])->sum('visit_total');
			$result['list'][] = $v;
		}
