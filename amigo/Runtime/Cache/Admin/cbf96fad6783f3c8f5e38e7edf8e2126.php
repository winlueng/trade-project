  ...
  <div id="kbVipAdmin-edit" class="spAnDynamic-edit" >
            <form class="" action="" method="post" enctype="multipart/form-data">
              <div class="kbVipAdmin-edit">
                ...
                <input type="text" value="<?php echo ($info["news_name"]); ?>" name="news_name" placeholder="请输入文章标题" maxlength="40"  required="required" data-datatype="input"  />
                <strong class="kbVipAdmin_tip">必填项。文章标题输入的字符不可以超过40个</strong>
                </p>
                <p class="kbVipAdmin_enter">
                    <b>文章作者：</b>
                    <input type="text" value="<?php echo ($info["news_author"]); ?>" name="news_author" placeholder="请输入文章作者" maxlength="8"  data-datatype="input" />
                    <strong class="kbVipAdmin_tip">选填项。文章作者输入字符不可超过8个</strong>
                </p>
                <!-- <p class="kbVipAdmin_enter">
                    <b><i>*</i>跳转路径：</b>
                    <input class="goHref" type="text" value="<?php echo ($info["news_link"]); ?>" name="news_link" placeholder="请输入原文网址"  required="required"/>
                    <strong class="kbVipAdmin_tip">必填项。动态原文出处的跳转页面链接，必须以https或者http开头的页面链接</strong>
                </p> -->

                <p class="kbVipAdmin_enter"><b><i>*</i>简述：</b>
                <textarea type="text" value="" maxlength="20" name="title" placeholder="请输入文章简述" required="required" ><?php echo ($info["title"]); ?></textarea>
                <strong class="kbVipAdmin_tip">必填项。动态简述输入字符不可超过20个</strong>
                </p>
                <p class="kbVipAdmin_enter mark-changeFile"><i>*</i>
                <b>上传类型</b>
                  <label class="kb_choose">
                    <input type="radio" name="is_video" value="0" <?php echo ($info['is_video']==0?'checked':''); ?> v-on:click="changeFile" requred="required" />
                  ...
                  <span>图片</span>
                   <label class="kb_choose">
                    <input type="radio" name="is_video" value="1"  <?php echo ($info['is_video']==1?'checked':''); ?> v-on:click="changeFile" required="required" />
                    <span class="kb_choose-check kb_choose-check_radio h20">
                      <em class="kb_choose-check-default kb_choose-check_main h"></em>
                    </span>
                  </label>
                ...
                <!-- name="news_logo[]" -->
                  ...
                  <?php if($info.is_video==0): if(is_array($info["news_logo"])): $i = 0; $__LIST__ = $info["news_logo"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="shopImg businessFileImgList">
                   ...
                   <div class="fileImgMore">
                        ...
                        <div class="shopImg businessFileImgList kbVipAdmin_videoInfo">
                             <video controls="controls"  width="100%" height="100%" poster="20170214143451.png"  runat="server" 
                                id="VideoShow">
                                <source type="video/mp4" src="/Public<?php echo ($info["video"]); ?>" >
                            </video>
                        </div>
                ...
                <p class="kbVipAdmin_enter">
                    ...
                    <select class="kbVipAdmin_slt kbVipAdmin_sltblack" name="news_classify_id" required="required" data-datatype="select">
                        ...
                        <?php if(is_array($classify)): foreach($classify as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php echo ($v['id'] == $info['news_classify_id']? 'selected':''); ?> ><?php echo ($v["news_type_name"]); ?></option><?php endforeach; endif; ?>
                    </select>
                    <strong class="kbVipAdmin_tip">必填项。请选择动态的分类</strong>
                </p>
                 <p class="kbVipAdmin_enter">
                    <b>相关产品：</b>
                    <input type="text" value="<?php echo ($info["goods_name"]); ?>" readonly class="mark-sltGoods"  />
                    <input type="hidden" value="<?php echo ($info["goods_id"]); ?>" name="goods_id" />

                     <button type="button" class="kb_btn kb_btnMini kb_btn-main mark-delAboutGoods" <?php echo ($info["goods_name? '': 'disabled'"]); ?> >删除</button>
                </p>
                </div>
...
<script type="text/javascript" src="/Public/Portal/PortalJS/portal.js?version=9.7"></script>
	...
	<script type="text/javascript">
		...
		for(var i = 0;i < nav_a.length;i ++)
		{
			...
			if($('.left_nav a:eq('+i+')').attr('active') == '/Admin/News/newsUpdateByProject'+'.html')
			{
        ...
        $(function(){
            ...
            (function(){
              var app=new Vue({
                el:".kbVipAdmin-edit",
                data:{
                   isChangeFile:"<?php echo ($info['is_video']); ?>"
                },
                methods:{
