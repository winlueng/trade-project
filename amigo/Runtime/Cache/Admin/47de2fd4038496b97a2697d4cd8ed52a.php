...
<body>
	...
	<div class="kbVipAdmin_alert couponAreaList_alert">
		...
		<h3>添加区域</h3>
		<form class="couponBusiness_FormParent pt10" action="<?php echo U('regionAdd');?>" method="post">
				<p class="couponTrade kbVipAdmin_enter">
					<b>父级区域：</b>
					<select name="parent_id">
						<option value="">--请选择--</option>
						<option value="0">顶级区域</option>
						<?php if(is_array($list)): foreach($list as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" /><?php echo str_repeat('&nbsp;', $v['count']*2).'|--'.$v['region_name']; endforeach; endif; ?>
					</select> 
				</p>
	...
	<script type="text/javascript">
		$(function(){
			...
			$('.couponAreaListWirte ').on('click',function(){
				...
				var $this = $(this);
				var ajaxUrl='<?php echo U("Region/ajaxControl");?>'
				var action='<?php echo U("regionUpdate");?>?id='
				jsAjaxControl($this,{
					ajaxURL:ajaxUrl,
					ajaxBranch:'Common',
					params:{
...
<script type="text/javascript" src="/Public/Portal/PortalJS/portal.js?version=9.7"></script>
	...
	<script type="text/javascript">
		...
		for(var i = 0;i < nav_a.length;i ++)
		{
			...
			if($('.left_nav a:eq('+i+')').attr('active') == '/Admin/Region/regionAdd'+'.html')
			{
