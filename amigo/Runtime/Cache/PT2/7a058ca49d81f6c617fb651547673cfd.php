       ...
       <?php if(is_array($list)): foreach($list as $key=>$vo): ?><li class="kb_list-cells" data-goodsId="<?php echo ($vo['id']); ?>">
           <a  href="<?php echo U('goodsDetail', ['id' => $vo['id'], 'companyName' => $_GET['companyName']]);?>" class="kb_list-cell">
              ...
              <aside class="kb_list-cell-bd">
                  <h2 class="dl_goods-title mb10"><?php echo ($vo["goods_name"]); ?></h2>
                  <div class="kb_list-cell-brief">
                      <!-- <p class="tg_ft-price mb10">
                          距结束：6天12时30分
                      </p> -->
                      <?php if(($vo['is_discount'] == 1) AND ((int)$vo['sales_start_time'] < time()) AND ((int)$vo['sales_end_time'] > time())): ?><p class="tg_ft-price mb10">
...
<nav class="dl_goShopCar">
    <a class="icon-shoppCat <?php echo ($cartTotal?'dl_myself-order-point':''); ?>" href="<?php echo U('ShoppingCart/myCart');?>?companyName=<?php echo I('get.companyName');?>&c_com=l"></a>
</nav>
