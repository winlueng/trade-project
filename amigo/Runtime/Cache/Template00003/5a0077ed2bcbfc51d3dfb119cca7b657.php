...
购物车
  ...
  <form id="myCartForm" action="<?php echo U('PT2/OrderForm/unpaid');?>?way=2&companyName=<?php echo ($_GET['companyName']); ?>&tem=<?php echo MODULE_NAME;?>&com=c&c_com=<?php echo I('get.c_com');?>&gid=<?php echo I('get.gid');?>" method="post">
  <?php if(is_array($info["project"])): $i = 0; $__LIST__ = $info["project"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><ul class="kb_list">
      ...
      <?php if(is_array($v)): $i = 0; $__LIST__ = $v;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><article  class="kb_list-cell" >  
            <div class="kb_list-cell-ft flexLayout-center mr10">
              <label class="kb_choose">
                ...
                <span class="kb_choose-check kb_choose-check_radio">
                  <b class="kb_choose-check-default kb_choose-check_main"></b>
				    ...
				    <input type="hidden" value="<?php echo ($vo["true_price"]); ?>" name="price[<?php echo ($vo["id"]); ?>]" />
  ...
  <?php if(is_array($info["company"])): $i = 0; $__LIST__ = $info["company"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><ul class="kb_list">
      ...
      <li class="kb_list-cells">
      <?php if(is_array($v)): foreach($v as $ki=>$vo): if(is_int($ki)): ?><article  class="kb_list-cell" >  
            <div class="kb_list-cell-ft flexLayout-center mr10">
              <label class="kb_choose">
                <input type="checkbox" value="<?php echo ($vo["id"]); ?>" name="cart_id[<?php echo ($vo["id"]); ?>]" data-price="<?php echo ($vo["true_price"]); ?>" data-total="<?php echo ($vo['total']); ?>" data-cartid="<?php echo ($vo["id"]); ?>"  />
                <span class="kb_choose-check kb_choose-check_radio">
  ...
  <footer id="Footer" class="kb_footer ">
    <div class="tg_cell  h">
        ...
        <p class="tg_cell-ft h ">
          <button type="button"  class="kb_btn tg_btn-main  w100 tg_cell-bd h mark-mycartRemove" >删除</button>
...
<script>
  $(function(){
    ...
    $('.mark-lower,.mark-add').on('click',function(){
        ...
        cartid=$this.parents('.dl_chang-total').data('cartid').trim();
        prams={
          flag:'setTotal',
          id:cartid
        }
        ...
        $.ajax({
          type:"GET",
          url:"<?php echo U('PT2/ShoppingCart/ajaxControl');?>",
          data:prams,
          dataType:'json',
    ...
    $('.mark-parentAll input').on('click change ',function(){
      ...
      totalPrice=0;
      totalNum=0;
      Object.keys(locationChecked).forEach(function(elem,indexs){
       var locationCheckedCurrent = locationChecked.eq(elem);
       if(locationCheckedCurrent.length){
         var price=parseInt(locationCheckedCurrent.data('price').trim(),10);
         var number=parseInt(locationCheckedCurrent.data('total'),10);
    ...
    $('.kb_main .kb_list .kb_choose input').on('click change',function(){
     var $this,locationChecked,locationinput,inputlength,checklength,totalPrice,totalNum;
         $this=$(this);
        //是否店铺全选
         locationChecked=$this.parents('.kb_list').find('.kb_list-cells .kb_choose input:checked');
         locationinput=$this.parents('.kb_list').find('.kb_list-cells .kb_choose input');
         totalPrice=0;
         totalNum=0;
         Object.keys(locationChecked).forEach(function(elem,indexs){
          var locationCheckedCurrent = locationChecked.eq(elem);
