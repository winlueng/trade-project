...
class Api 
{
    ...
    protected $save_jsapi_ticket_diy;
     ...
     /** @var callable $get_jsapi_ticket_diy 用户自定义获取api_ticket的方法 */
    protected $get_api_ticket_diy;
    /** @var callable $save_jsapi_ticket_diy 用户自定义保存api_ticket的方法 */
    protected $save_api_ticket_diy;
    ...
    public function __construct($config)
    {
        ...
        $this->get_api_ticket_diy     	=   isset($config['get_api_ticket']) ? $config['get_api_ticket'] : false;
        $this->save_api_ticket_diy    	=   isset($config['save_api_ticket']) ? $config['save_api_ticket'] : false;
    }
    ...
    public function get_access_token()
    {
        ...
        if ($this->get_access_token_diy !== false) {
            ...
            $token = call_user_func($this->get_access_token_diy, $this->appId);
            if ($token) {
        ...
        if (!$this->valid_access_token($token)) {
            ...
            if ($this->save_access_token_diy !== false) {
                ...
                call_user_func($this->save_access_token_diy, $this->appId,json_encode($token));exit;
            } else {
    ...
    /**
     * 获取模版ID
     * @param  [type] $templateID 模版编号
     * @return jsonp             模版ID
     */
    public function get_template_id($templateID)
    {
     ...
     */
    public function send_template_news($openID, $templateID, $news, $url = '')
    {
        $data = [
            'touser' => $openID,
        ...
        // return $data;
        $res = HttpCurl::post($url, $data, 'json');
        // 异常处理: 获取access_token网络错误
        if ($res === false) {
            return Error::code('ERR_GET');
        }
     ...
     * @return mixed
     */
    public function new_api_ticket()
    {
        $url = self::API_DOMAIN . 'cgi-bin/ticket/getticket?access_token=' . $this->get_access_token() . '&type=wx_card';
        $res = HttpCurl::get($url, 'json');
        // 异常处理: 获取时网络错误
        if ($res === false) {
            return Error::code('ERR_GET');
    ...
    //添加卡券
	public function add_Card($CardID) {
	    $timestamp = time();
	    $nonceStr  = $this->createNonceStr();
	    $apiTicket = $this->get_api_ticket();
     ...
     * @return bool
     */
    public function valid_api_ticket($ticket)
    {
    ...
    {
        $ticket = false;
        if ($this->get_api_ticket_diy !== false) {
            // 调用用户自定义获取jsapi_ticket方法
            $ticket = call_user_func($this->get_api_ticket_diy, $this->appId);
            if ($ticket) {
        ...
        // 验证jsapi_ticket是否有效
        if (!$this->valid_api_ticket($ticket)) {

            // 生成新的jsapi_ticket
            $ticket = $this->new_api_ticket();
            if ($ticket === false) {
                return false;
            }

            // 保存新生成的AccessToken
            if ($this->save_api_ticket_diy !== false) {
                // 用户自定义保存AccessToken方法
                call_user_func($this->save_api_ticket_diy, $this->appId, json_encode($ticket));
            } else {
                // 异常处理: 保存access_token方法未定义
    ...
    public function get_card_list($start = 0, $count = 10)
    {
        $url = self::API_DOMAIN . 'card/batchget?access_token=' . $this->get_access_token();
        $json = '{
                  "offset": '. $start .',
                  "count": '. $count .', 
        ...
        // 判断是否调用成功
        if ($res->errcode) {
            return array(null, $res->errcode);
        } else {            
            return $res;
        }  
    }

    public function get_card_info($json, $info='')
    {
        $url = self::API_DOMAIN . 'card/get?access_token=' . $this->get_access_token();
        ...
        if ($res['errcode']) {
            ...
            if ($info == 'base_info') {
                $type = strtolower($res['card']['card_type']);
                return $res['card'][$type]['base_info'];
            }
            return $res;
        }
    }
    ...
    public function get_jsapi_ticket()
    {
        ...
        if ($this->get_jsapi_ticket_diy !== false) {
            // 调用用户自定义获取jsapi_ticket方法
            $ticket = call_user_func($this->get_jsapi_ticket_diy, $this->appId);
            if ($ticket) {
                $ticket = json_decode($ticket);
            }
        } else {
        ...
        if (!$this->valid_jsapi_ticket($ticket)) {
            ...
            if ($this->save_jsapi_ticket_diy !== false) {
                // 用户自定义保存AccessToken方法
                call_user_func($this->save_jsapi_ticket_diy, $this->appId, json_encode($ticket));
            } else {
                // 异常处理: 保存access_token方法未定义
                @error_log('Not set saveTokenDiy method, jsapi_ticket will be refreshed each time.', 0);
            }
